...
const viewClassStudents = async (classId) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3001/api/teacher/classes/${classId}/students`,
      {
        headers: {
          'user-id': sessionStore.user.id
        }
...
const removeStudentFromClass = async (studentId) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3001/api/teacher/classes/${selectedClassId.value}/students/${studentId}`,
      {
        method: 'DELETE',
        headers: {
          'user-id': sessionStore.user.id
